...
const CamarasTable = () => {
  ...
  const camarasState = useSelector((state) => state.camarasReducer);

  const camaras = camarasState.currentList;
  const camarasCount = camarasState.count;
  const limit = camarasState.limit;
  const offset = camarasState.offset;
  const page = camarasState.page;

  //filters
  const filterName = camarasState.filters.filterName;
  const filterUnits = camarasState.filters.filterUnits;
  const filterAverageValues = camarasState.filters.filterAverageValues;
  const filterQualifications = camarasState.filters.filterQualifications;
  const filterCity = camarasState.filters.filterCity;
  const filterUf = camarasState.filters.filterUf;
  ...
  const data =
    (camaras &&
      camaras.map((c) => [
        c.nome_fantasia,
        c.actuation_units.join(", "),
        c.actuation_cities.join(", "),
        <Rating
          name="half-rating"
          ...
          readOnly
        />,
        getAverageValueLabel(c.average_value),
      ])) ||
    [];
  ...
  const options = {
    ...
    onRowClick: (rowData, rowMeta) => {
      history.push(`/perfil/publico/camara/${camaras[+rowMeta.dataIndex].id}`);
    },
  };
